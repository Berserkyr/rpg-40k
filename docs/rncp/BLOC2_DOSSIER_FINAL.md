...
| C2.2.2 | [Harnais de tests unitaires + couverture](HARNAIS_TESTS_UNITAIRES.md) |
| C2.2.3 | [Mesures sécurité + accessibilité](MESURES_SECURITE_ACCESSIBILITE.md) |
...
| `VPS_SSH_KEY_B64` | clé privée SSH dédiée encodée en base64 |
| `VPS_PORT` | port SSH |
...
Le livrable dédié est disponible ici : [MESURES_SECURITE_ACCESSIBILITE.md](MESURES_SECURITE_ACCESSIBILITE.md).

La sécurité est analysée selon **OWASP Top 10 (2021)**.

| Risque OWASP | Mesure mise en œuvre ou positionnement |
|---|---|
| Broken Access Control | routes protégées par JWT, utilisateur courant résolu côté backend, rôle `admin` pour les endpoints sensibles |
| Cryptographic Failures | mots de passe hachés avec `bcrypt`, JWT signés, secrets externalisés |
| Injection | entrées backend typées par Pydantic, absence de SQL dynamique utilisateur |
| Insecure Design | séparation frontend/backend/domaine, sessions isolées, persistance par utilisateur |
| Security Misconfiguration | `.dockerignore`, `.gitignore`, variables d’environnement, port dédié VPS, CI avant production |
| Vulnerable Components | dépendances déclarées et installées en CI |
| Authentication Failures | vérification du mot de passe, bearer token, expiration et refus `401/403` |
...
| ARIA | `aria-label`, `role="group"`, `role="alert"` sur les interactions principales |
| Navigation simple | actions regroupées par panneaux |
| Feedback utilisateur | terminal affichant erreurs et résultats, boutons désactivés si action impossible |
...
Le livrable dédié est disponible ici : [HARNAIS_TESTS_UNITAIRES.md](HARNAIS_TESTS_UNITAIRES.md).
